...
import { KEY_CURRENT, KEY_HISTORY, KEY_SETTINGS } from "../shared/storage-keys";

describe("State loader extraction", () => {
  const stateLoadersPath = resolve("src/logic/state-loaders.ts");
  const mainSource = readFileSync(resolve("src/main.tsx"), "utf8");

  it("keeps localStorage state loaders outside main.tsx", async () => {
    expect(existsSync(stateLoadersPath)).toBe(true);

    const modulePath = "../logic/state-loaders";
