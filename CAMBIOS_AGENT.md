...
export const AddEntryScreen: FC<AddEntryScreenProps> = ({
  ...
  // ... JSX del teclado, display de importe, campo de nota y botón guardar
};
```
...
#### Código anterior
```tsx
import { AddEntryScreen } from "./screens/add-entry-screen";  // ← import añadido
// y después:
if (screen === "add") {
  const setVal = activeField === "propina" ? setValP : setValD;
  // function kpAdd, function handleSaveAdd, JSX completo del teclado datapropina
}
```
...
#### Código anterior
```tsx
import { AddNotaGeneralScreen } from "./screens/add-nota-general-screen";  // ← import añadido
// y después:
...
#### Por qué se cambió
La pantalla de entrada individual es una responsabilidad autocontenida (teclado numérico para añadir Agency/Bono, Extra, Gasolina o Nulo). Extraerla a `src/screens/add-single-entry-screen.tsx` reduce `main.tsx` en ~78 líneas y la convierte en una unidad reutilizable con frontera clara.

### Cambio 2 - Reemplazar bloque addSingle en main.tsx

#### Código anterior
```tsx
import { AddSingleEntryScreen } from "./screens/add-single-entry-screen";  // ← import añadido
// y después:
...
#### Código anterior
```tsx
// Inline en main.tsx (~12 líneas de definition por cada icono)
```
...
#### Por qué se cambió
Los iconos inline se sustituyen por los imports de los componentes extraídos. Comportamiento idéntico.
...
#### Por qué se cambió
Las definiciones inline se sustituyen por los imports del módulo extraído. Comportamiento idéntico.
